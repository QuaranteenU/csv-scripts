...
pretty_order = [{'Name': item['Name'], 'Start Time': (datetime.strptime(item['Start Time'], '%Y-%m-%d %I:%M:%S %p') + timedelta(hours=emailToTz[item['Email']])).strftime('%Y-%m-%d %I:%M:%S %p')} for item in studentsWithTZ]
if PRINT_TABULATE:
...
for entry in data:
  ...
  if school != cur_school:
    ...
    schoolOrder.append({
      'School': school,
      'Start Time': datetime.strptime(entry['Start Time'], '%Y-%m-%d %I:%M:%S %p')
    })

schoolOrder = sorted(schoolOrder, key = lambda i: i['Start Time'])
schoolOrder = [{'School': s['School'], 'Start Time': s['Start Time'].strftime('%Y-%m-%d %I:%M:%S %p')} for s in schoolOrder]

# save utc school schedule
with open('data/FINAL school schedule utc.csv', 'w', encoding="utf-8", newline='') as schoolScheduleCSVFile:    
